...
function addWatch(watchPath, handle, debounceTime) {
    ...
    return watches[watchPath] = fs.watch(watchPath, { persistent: false }, function(type, filename) {
            ...
            if(type === 'change') {
                ...
                if(fs.existsSync(filename)) {
                    ...
                    if(fs.statSync(filename).isDirectory()) {
                        addWatch(filename, handle, debounceTime);
                    }
        ...
        pendingEvents[filename] = setTimeout(handleEvent, debounceTime || 100);
    });
}

module.exports = function(pathsToWatch, handle, debounceTime) {
    for(var i = 0, stat, watchPath; i < pathsToWatch.length; i++) {
        ...
        if(!watches[watchPath]) {
            addWatch(watchPath, handle, debounceTime);
        }
    }
};
