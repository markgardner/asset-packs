...
function BaseCompiler(opts) {
	...
	this.sync = opts.sync;
	this.useCoverage = opts.useCoverage;
	...
	globFiles: function(patterns, baseDir, generatedPath) {
		...
		function generateFileList() {
			...
			for(i = patterns.length - 1; i >= 0; i--) {
				...
				newFiles = glob.sync(globPath).concat(newFiles);
			}

			// Remove duplicates from the beginning
			newFiles = newFiles.filter(function(item, idx) { 
				return newFiles.indexOf(item) === -1 || newFiles.lastIndexOf(item) === idx;
			});
